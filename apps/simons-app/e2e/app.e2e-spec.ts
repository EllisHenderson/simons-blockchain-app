...
describe('Starting tests for simons-app', function() {
  ...
  it('website title should be simons-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('simons-app');
    })
  });

  it('network-name should be simons-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('simons-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be simons-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('simons-app');
    });
  });

  
    it('SimonsAsset component should be loadable',() => {
      page.navigateTo('/SimonsAsset');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('SimonsAsset');
      });
    });

    it('SimonsAsset table should have 4 columns',() => {
      page.navigateTo('/SimonsAsset');
